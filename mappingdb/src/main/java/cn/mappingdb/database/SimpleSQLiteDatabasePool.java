...
public class SimpleSQLiteDatabasePool implements Serializable {

    private static final long serialVersionUID = 190512148596251207L;
    ...
    private SimpleSQLiteDatabase.DBParams params;
    private SimpleSQLiteDatabase.DBUpdateListener mDBUpdateListener; // 升级时监听器
    private Boolean isWrite = false;
    private static HashMap<String, SimpleSQLiteDatabasePool> poolMap = new HashMap<String, SimpleSQLiteDatabasePool>();
     ...
     */
    public synchronized static SimpleSQLiteDatabasePool getInstance(
            Context context, SimpleSQLiteDatabase.DBParams params, Boolean isWrite) {
        String dbName = params.getDbName().trim();
        SimpleSQLiteDatabasePool pool = poolMap.get(dbName);
        if (pool == null) {
            pool = new SimpleSQLiteDatabasePool(context, params, isWrite);
     ...
     * @param context 上下文
     */
    public static SimpleSQLiteDatabasePool getInstance(Context context) {
        SimpleSQLiteDatabase.DBParams params = new SimpleSQLiteDatabase.DBParams();
    ...
    public static SimpleSQLiteDatabasePool getInstance(Context context,
        ...
        SimpleSQLiteDatabase.DBParams params = new SimpleSQLiteDatabase.DBParams(dbName, dbVersion);
     ...
     * @param isWrite 打开数据库如果是 isWrite为true,则磁盘满时抛出错误
     */
    public SimpleSQLiteDatabasePool(Context context, SimpleSQLiteDatabase.DBParams params,
                                    Boolean isWrite) {
     ...
     */
    public void setOnDbUpdateListener(SimpleSQLiteDatabase.DBUpdateListener dbUpdateListener) {
     ...
     * 获取升级的的监听器
     */
    public SimpleSQLiteDatabase.DBUpdateListener getOnDbUpdateListener() {
        return mDBUpdateListener;
    }
        ...
        for (int x = 0; x < numSQLiteDatabase; x++) {
            ...
            try {
                ...
                if (getOnDbUpdateListener() == null) {
                    ...
                    pSQLiteDatabases.addElement(new PooledSQLiteDatabase(newSQLiteDatabase(getOnDbUpdateListener())));
                }
            } catch (Exception e) {
...
//			TALogger.i(TASQLiteDatabasePool.this, "数据库连接己创建 ......");
    ...
    private SimpleSQLiteDatabase newSQLiteDatabase() {

        // 创建一个数据库连接
        SimpleSQLiteDatabase sqliteDatabase = new SimpleSQLiteDatabase(context, params);
    ...
    private SimpleSQLiteDatabase newSQLiteDatabase(SimpleSQLiteDatabase.DBUpdateListener tadbUpdateListener) {

        // 创建一个数据库连接
        SimpleSQLiteDatabase sqliteDatabase = new SimpleSQLiteDatabase(context, params, tadbUpdateListener);
        sqliteDatabase.openDatabase(mDBUpdateListener, isWrite);
        return sqliteDatabase; // 返回创建的新的数据库连接
    }
    ...
    public synchronized SimpleSQLiteDatabase getSQLiteDatabase() {
        // 确保连接池己被创建
        if (pSQLiteDatabases == null) {
            return null; // 连接池还没创建，则返回 null
        }

        SimpleSQLiteDatabase sqliteDatabase = getFreeSQLiteDatabase(); // 获得一个可用的数据库连接
    ...
    private SimpleSQLiteDatabase getFreeSQLiteDatabase() {
        // 从连接池中获得一个可用的数据库连接
        SimpleSQLiteDatabase sqLiteDatabase = findFreeSQLiteDatabase();
    ...
    private SimpleSQLiteDatabase findFreeSQLiteDatabase() {
        SimpleSQLiteDatabase sqliteDatabase = null;
     ...
     */
    private boolean testSQLiteDatabase(SimpleSQLiteDatabase sqliteDatabase) {
    ...
    public void releaseSQLiteDatabase(SimpleSQLiteDatabase sqLiteDatabase) {
        // 确保连接池存在，如果连接没有创建（不存在），直接返回
        if (pSQLiteDatabases == null) {
            DaoLog.d(" 连接池不存在，无法返回此连接到连接池中 !");
            return;
        }
        PooledSQLiteDatabase pSqLiteDatabase = null;
    ...
    private void closeSQLiteDatabase(SimpleSQLiteDatabase sqlLiteDatabase) {
    ...
    class PooledSQLiteDatabase {
        SimpleSQLiteDatabase sqliteDatabase = null;// 数据库连接
        boolean busy = false; // 此连接是否正在使用的标志，默认没有正在使用

        // 构造函数，根据一个 TASQLiteDatabase 构告一个 PooledSQLiteDatabase 对象
        public PooledSQLiteDatabase(SimpleSQLiteDatabase sqliteDatabase) {
            this.sqliteDatabase = sqliteDatabase;
        }

        // 返回此对象中的连接
        public SimpleSQLiteDatabase getSqliteDatabase() {
            return sqliteDatabase;
        }

        // 设置此对象的，连接
        public void setSqliteDatabase(SimpleSQLiteDatabase sqliteDatabase) {
            this.sqliteDatabase = sqliteDatabase;
        }
